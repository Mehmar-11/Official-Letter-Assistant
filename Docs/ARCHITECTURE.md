...
| Boundary | Control |
|---|---|
| Before analysis | Input verification rejects non-letter content (`is_valid_letter: false`) |
| After LLM analysis | Pydantic validates the structured output schema |
| After follow-up | Pydantic validates the follow-up response format |
| After chat | Exception handler returns HTTP 502 with a controlled error message |
