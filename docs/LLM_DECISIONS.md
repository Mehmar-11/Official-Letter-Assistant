...
**Decision**: No `language` parameter in the `/chat` endpoint. The model replies in whatever language the user writes in.

**Why**: A separate language parameter creates a synchronization problem — the user might select "Turkish" in a dropdown but write in English, or switch languages mid-conversation. Language detection from the message itself is more reliable and requires no extra user input. GPT-4o handles this accurately across the languages relevant to the target user group (German, English, Persian, Turkish, Arabic).

**Trade-off**: The system cannot enforce a specific output language. This is acceptable — the goal is for the user to get a response in their natural language, not a language chosen by a UI control.
